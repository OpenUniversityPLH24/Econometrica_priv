...
//        plot.setDefaultCloseOperation(MainForm.DISPOSE_ON_CLOSE);
        ...
        em.close();
            ...
            try (Response response2 = client.newCall(request2).execute()) {
                if(response2.isSuccessful() && response2.body() != null) {
                    ...
                    countryDatasetOIL.setDescription(JsonOil.getDataset().getDescription());                        // description
                    countryDatasetOIL.setName(JsonOil.getDataset().getName());                                      // name
                    countryDatasetOIL.setStartYear(((String)JsonOil.getDataset().getStartDate()).substring(0, 4)); // start date
                    countryDatasetOIL.setEndYear(((String)JsonOil.getDataset().getEndDate()).substring(0, 4));     // end date
                // Ελέγχω αν το dataset εχει δεδομενα
                    ...
                    for(i=0;i<oil;i++){
                        ...
                        countryData.setDataYear((String)OilDataTable.getValueAt(i, 0)); // θέτω έτος
                        countryData.setValue((String) (OilDataTable.getValueAt(i, 1))); // , αξία και
                        countryData.setDataset(countryDatasetOIL); // dataSet object στην δομή countryData
                        countryDataOILst.CountryDataLstAdd(countryData);
                        System.out.println(countryDataOILst.getLst().get(i).getValue());
                        System.out.println(countryDataOILst.getLst().get(i).getDataYear());
            ...
            try (Response response1 = client.newCall(request1).execute()) {
                if(response1.isSuccessful() && response1.body() != null) {
                    ...
                    countryDatasetGDP.setDescription(JsonGdp.getDataset().getDescription());                        // description
                    countryDatasetGDP.setName(JsonGdp.getDataset().getName());                                      // name
                    countryDatasetGDP.setStartYear(((String)JsonGdp.getDataset().getStartDate()).substring(0, 4)); // start date
                    countryDatasetGDP.setEndYear(((String)JsonGdp.getDataset().getEndDate()).substring(0, 4));     // end date
                // Ελέγχω αν το dataset εχει δεδομενα
                    ...
                    for(i=0;i<gdp;i++){
                        ...
                        countryData = new CountryData();
                        countryData.setDataYear((String)GPDDataTable.getValueAt(i, 0)); // θέτω έτος
                        countryData.setValue((String) (GPDDataTable.getValueAt(i, 1))); // , αξία και
                        countryData.setDataset(countryDatasetOIL); // dataSet object στην δομή countryData
                        countryDataOILst.CountryDataLstAdd(countryData);
                    }
                }
    ...
    private void Save_ButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Save_ButtonActionPerformed
        if (JPA.retreiveCountry((String) CountrySelect.getSelectedItem()) == null) {
             System.out.println("Zero check - country is = " + country.getIsoCode());
             JPA.createCountry(country); // Αποθηκεύω την χώρα στην βάση
             System.out.println("1st check - country created");
                  System.out.println("countryDatasetLst count = " + countryDatasetLst.size());
                  System.out.println("GDP entry found ? = " + countryDatasetLst.get(1).getDescription().contains("GDP"));
                  
                  for (int j=0; j< countryDatasetLst.size(); j++ ) {
                       JPA.createCountryDataset(countryDatasetLst.get(j));
                       if (countryDatasetLst.get(j).getDescription().contains("Oil")) {
